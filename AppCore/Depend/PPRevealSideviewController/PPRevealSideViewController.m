...
- (void) pushViewController:(UIViewController *)controller onDirection:(PPRevealSideDirection)direction withOffset:(CGFloat)offset animated:(BOOL)animated forceToPopPush:(BOOL)forcePopPush {
    ...
    if (!PPSystemVersionGreaterOrEqualThan(5.0)) [controller viewWillAppear:animated];{
        if (type==PPRevealSideOnObove) {
            oboveTypeDirection=direction;
            _isOboveClose=YES;
           [self.view insertSubview:controller.view aboveSubview:_rootViewController.view];
        }else{
        ...
        if (finished) {
            ...
            void (^smallAnimBlockCompletion)(BOOL) = ^(BOOL finished) {
                if (finished) {
                    ...
                    _animationInProgress = NO;
                    if (_isOboveClose&&type==PPRevealSideOnObove){
                        _isOboveClose=NO;oboveTypeDirection = 0;
                    }
                    if (controllerToPush) {
...
- (void) changeOffset:(CGFloat)offset forDirection:(PPRevealSideDirection)direction animated:(BOOL)animated {
    [self setOffset:offset forDirection:direction];
    if (type==PPRevealSideOnObove) return;
...
- (void) addShadow
{
    ...
    _rootViewController.view.layer.shadowRadius = 0.0f /*5.0f*/;
...
- (BOOL) isLeftControllerClosed {
    if (type==PPRevealSideOnObove&&oboveTypeDirection==PPRevealSideDirectionLeft)
        return !_isOboveClose;
    return CGRectGetMinX(_rootViewController.view.frame) <= 0;
}

- (BOOL) isRightControllerClosed {
    if (type==PPRevealSideOnObove&&oboveTypeDirection==PPRevealSideDirectionRight)
        return !_isOboveClose;
...
- (void) gestureRecognizerDidPan:(UIPanGestureRecognizer*)panGesture {
    if(self.type==PPRevealSideOnObove) return;  
